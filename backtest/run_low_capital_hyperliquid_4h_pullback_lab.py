"""Native Hyperliquid 4h trend-pullback momentum with state-only exits."""
...
def classifier(candidate, prices, funding_rate, index, holding):
    ...
    if not holding:
        if trend < candidate["minimum_trend"]:
...
def main():
    ...
    parser.add_argument("--fixture", default="data/clean_room/hyperliquid_4h_low_capital_pullback.json")
    parser.add_argument("--output", default="data/research_artifacts/low_capital_hyperliquid_4h_pullback.json")
    ...
    if holdout_pass:
        ...
        capital_replay = {
            ...
            for capital in (10, 15, 20, 21, 22, 23, 24, 25, 30, 50, 100)
        }
    artifact = {
        "schema_version": 1,
        "execution_authorized": False,
        "route": "native_hyperliquid_4h_trend_pullback_reclaim",
        "holding_rule": "no minimum or maximum duration; exit only on recovery or trend-state decay",
