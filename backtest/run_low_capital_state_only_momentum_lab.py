"""Momentum with no holding-time rule: every transition is market-state driven."""
...
VALIDATION_START = (60 + 180 + 30) * DAY_BARS
VALIDATION_BARS = 180 * DAY_BARS
...
def main():
    ...
    parser.add_argument("--output", default="data/research_artifacts/low_capital_state_only_momentum.json")
    ...
    validation = (VALIDATION_START, VALIDATION_START + VALIDATION_BARS)
    if validation[1] >= development_start:
        raise ValueError("state-only validation overlaps development")
    evaluated = []
    ...
    route = {
        "route": "state_only_regime_momentum",
        "hypothesis": "holding while measured momentum state persists is more robust than any elapsed-time exit",
    ...
    if args.unlock_validation and passing:
        route["validation"] = simulate(best["candidate"], timestamps, closes, funding, payload["sz_decimals"], start=validation[0], end=validation[1])
        route["validation_stressed"] = simulate(best["candidate"], timestamps, closes, funding, payload["sz_decimals"], start=validation[0], end=validation[1], cost_bps=10.0)
        failures = []
        if route["validation"]["net_return_pct"] <= 0:
            failures.append("normal_validation_net_return_not_positive")
        if route["validation_stressed"]["net_return_pct"] <= 0:
            failures.append("stressed_validation_net_return_not_positive")
        if route["validation_stressed"]["max_drawdown_pct"] > 20.0:
            failures.append("stressed_validation_drawdown_above_20_pct")
        route["decision"] = "rejected_in_validation" if failures else "validation_pass"
    ...
    artifact = {
        ...
        "data": {"development_start": timestamps[development_start], "development_end": timestamps[development_end - 1], "known_latest_excluded_start": timestamps[development_end], "validation_start": timestamps[validation[0]], "validation_end": timestamps[validation[1] - 1]},
        "route": route,
    }
    output = Path(args.output)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(artifact, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(json.dumps({"decision": route["decision"], "passing_candidates": route["passing_candidates"], "best": best["candidate"], "normal": best["normal"], "stressed": best["stressed"], "review": route["review"], "validation": route.get("validation")}, indent=2, sort_keys=True))
