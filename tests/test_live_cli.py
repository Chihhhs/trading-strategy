...
class LiveCliTest(unittest.TestCase):
    ...
    @patch("trading_strategy.live.cli.ensure_position_protection")
    @patch("trading_strategy.live.cli.record_trade_event")
    def test_run_once_uses_specific_protection_blocker_counts_in_summary(
        self,
        mock_record_trade_event,
        mock_ensure_position_protection,
        mock_check_entries,
        ...
        try:
            ...
            mock_ensure_position_protection.return_value = {
                ...
                "protection_missing_count": 1,
                "tpsl_missing_count": 1,
                "tpsl_repaired_count": 0,
                "unprotected_positions_count": 5,
                "ambiguous_protection_count": 2,
                "verification_unknown_count": 1,
                "protection_repair_failed_count": 1,
                "protection_update_failed_count": 1,
            }
            with patch("trading_strategy.live.cli.sync_state_with_hl_balance", side_effect=lambda state: state):
                cli.run_once()
            mock_check_entries.assert_not_called()
            ...
            self.assertEqual(
                run_summary_call.kwargs["top_blockers"],
                [
                    {"reason": "ambiguous_protection", "count": 2},
                    {"reason": "verification_unknown", "count": 1},
                    {"reason": "protection_repair_failed", "count": 1},
                    {"reason": "protection_update_failed", "count": 1},
                    {"reason": "missing_protection", "count": 1},
                ],
            )
            self.assertTrue(run_summary_call.kwargs["has_unprotected_positions"])
            self.assertEqual(run_summary_call.kwargs["ambiguous_protection_count"], 2)
            self.assertEqual(run_summary_call.kwargs["verification_unknown_count"], 1)
        finally:
            live.config.set_mode(old_mode)
    ...
    @patch("trading_strategy.live.cli.ensure_position_protection")
    @patch("trading_strategy.live.cli.record_trade_event")
    def test_run_once_blocks_entries_for_ambiguous_or_verification_unknown_protection(
        self,
        mock_record_trade_event,
        mock_ensure_position_protection,
        mock_check_entries,
        ...
        try:
            ...
            mock_ensure_position_protection.return_value = {
                ...
                "tpsl_repaired_count": 0,
                "unprotected_positions_count": 2,
                "ambiguous_protection_count": 1,
                "verification_unknown_count": 1,
            ...
            mock_check_entries.assert_not_called()
            run_summary_call = next(call for call in mock_record_trade_event.call_args_list if call.args[0] == "run_summary")
            self.assertEqual(run_summary_call.kwargs["unprotected_positions_count"], 2)
            self.assertEqual(run_summary_call.kwargs["top_blockers"][0]["reason"], "ambiguous_protection")
            self.assertEqual(run_summary_call.kwargs["top_blockers"][1]["reason"], "verification_unknown")
        finally:
            live.config.set_mode(old_mode)
