...
| P1 | Live decision architecture | Implemented in observe-only mode. Each signal path emits a reason-coded `Decision` event and summary aggregation without changing entry behavior. Paper-mode K-line cache may resolve pending observations while offline; it never supplies a live decision or entry price. | Collect normal-run evidence; activate nothing without a separate research gate and live-safety review. | `src/trading_strategy/live/decision.py`, `src/trading_strategy/live/market.py`, live entry tests |
