...
| P1 | Trend strategy | `optimize_existing_trend`: executable, but not validated as live alpha under a 50-coin causal replay baseline. | Start with raw Trend entry attribution and fixed walk-forward evidence; only then test one pre-defined entry hypothesis. | `.agents/research_modes.md`, `.agents/improve_plan.md` |
| P1 | Trend market context | `optimize_existing_trend` research-only candidate: causal regime entry filter plus momentum-decay time limit. | Re-run only as 50-coin diagnostic; no shadow or promotion until a 50-coin 1h replay fixture exists. | `.agents/research_modes.md`, `docs/research_manual/09_trend_market_context_candidate.md` |
