...
| P1 | Live decision architecture | Implemented in observe-only mode. Each signal path emits a reason-coded `Decision` event and summary aggregation without changing entry behavior. | Collect normal-run evidence; activate nothing without a separate research gate and live-safety review. | `src/trading_strategy/live/decision.py`, live entry tests |
