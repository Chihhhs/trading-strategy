...
| P1 | Clean-room 4h selector | Route 30 and the weaker volume-state Route 31 remain the validated 50-USDC paper baselines. Trade attribution showed initial failures dominate losing PnL; routes 32-34 failed the repeatedly inspected OOS benchmark, Route 35 found no incremental value from persistent volume/volatility expansion, and Route 36 found no development-consistent improvement from BTC-residual or cross-sectional excess momentum. Routes 38-40 are user-approved, isolated entry-quality forward ledgers on the new boundary; their known-fixture diagnostic replay did not beat Route 30, and Route 40 breached the stress drawdown boundary. Routes 41-44 are additional backtest-only hypotheses from the same review; they beat Route 30 on the inspected holdout, but this is not fresh OOS evidence. | Keep Route 30/31 unchanged. Keep Routes 38-44 research-only. Do not select or wire any route from the inspected fixture or current forward sample; first rerun Routes 30/41/42/43 on a newly fingerprinted OOS boundary under the same costs and gate. `execution_authorized` remains false. | `docs/research_manual/15_low_capital_route_log_2026-07-18.md`, `docs/research_manual/18_entry_quality_routes_2026-07-24.md`, `docs/research_manual/19_entry_quality_followup_2026-07-24.md`, `data/research_artifacts/backtesting_py_live38_4h_entry_quality_routes_2026-07-24.json` |
| P1 | Low-capital snowball research | Route 37's first clean-room feasibility baseline used a new 38-coin Hyperliquid snapshot. At 50 USDC it returned +3.97% under 10 bps stress on the locked 300-bar holdout, with -18.44% MTM drawdown and zero minimum-order skips; 25 USDC failed with -5.66% and 200 skips. The canonical live diagnostic was negative but used a different Binance fixture, timeframe, capital, leverage, and cost contract, so Route 37 has not yet been proven better than live. | First produce a same-input, matched-contract live-like comparison. Then run the predeclared 25/50/75/90% sizing grid only on a newly fingerprinted forward boundary; do not select on the inspected holdout. No paper/live wiring, exchange orders, or `execution_authorized=true`. | `docs/research_manual/16_low_capital_snowball_route_2026-07-22.md`, `data/research_artifacts/route37_capital_feasibility_baseline_holdout_2026-07-22.json`, `experiments/live_trend_baseline_38.json` |
