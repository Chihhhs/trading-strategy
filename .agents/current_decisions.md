...
| Complete | Module cleanup | Removed obsolete app/versioned wrappers and `core` re-export modules. Shared helpers now live in `shared/`, strategy hooks in `strategies/`, and position helpers in `positions/`. | Keep canonical runner commands and persisted schemas stable. Retain `core/legacy_unified.py` while the public `legacy_unified` negative-control strategy exists. | `docs/restruct.md`, live/backtest tests |
