...
- Entry quality: Routes 38-40 are isolated forward ledgers for two-bar leader persistence, a higher switch margin, and a longer trend confirmation. Routes 41-44 are backtest-only follow-ups for momentum floors, volatility-normalized ranking, and re-entry cooldown; they beat Route 30 on the inspected holdout but are not fresh OOS evidence. Keep Route 30/31 unchanged, validate 30/41/42/43 on a newly fingerprinted boundary, and preserve `execution_authorized=false` until the documented forward gate is met.
